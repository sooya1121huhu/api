...
### Fragrantica 스크래핑 (신규)
- `POST /api/scrape/perfume` - 단일 향수 스크래핑
- `POST /api/scrape/brand` - 브랜드별 향수 목록 스크래핑
- `POST /api/scrape/retry-failed` - 실패한 향수 재시도 스크래핑
- `POST /api/scrape/cancel/:jobId` - 스크래핑 작업 중단
- `POST /api/scrape/cancel-all` - 모든 스크래핑 작업 중단
- `GET /api/scrape/status/:jobId` - 스크래핑 작업 상태 조회
- `GET /api/scrape/jobs` - 스크래핑 작업 목록 조회
...
### Fragrantica 스크래핑 (신규)
- Fragrantica.com에서 향수 정보 자동 수집
- 단일 향수 또는 브랜드별 배치 스크래핑 (모든 향수 자동 수집)
- 자동 재시도 로직 (타임아웃 시 최대 2회 재시도)
- 실패한 향수 재시도 API (별도 엔드포인트)
- 자동 중복 검사 및 브랜드 생성
- 노트 정보 정규화 (영어 → 한국어)
- 실시간 진행률 추적
...
### 추천 시스템
- 노트 유사성 매핑 시스템 (베르가못/버가못, 로즈/장미 등 유사 노트 그룹화)
- 노트 기반 유사 향수 추천 (2개 이상 공통 노트)
- 계절/날씨 태그 기반 추천
